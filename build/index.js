...
async function handleToolCall(name, args) {
    switch (name) {
        ...
        case 'whatsapp_get_messages': {
            const { contactName, phoneNumber, limit = 100 } = args;
        ...
        case 'whatsapp_fetch_more_messages': {
            const { contactName, phoneNumber, count = 50 } = args;
            if (!contactName && !phoneNumber) {
                return JSON.stringify({ error: 'Either contactName or phoneNumber is required' });
            }
            if (!whatsappClient.isClientReady()) {
        ...
        case 'whatsapp_search_messages': {
            const { query, chatId, limit = 50 } = args;
            if (!query) {
        ...
        case 'whatsapp_download_media': {
            const { messageId, chatId, outputPath, analyze } = args;
            if (!messageId || !chatId) {
        ...
        case 'whatsapp_send_image': {
            const { recipient, imageUrl, imageBase64, caption } = args;
            if (!recipient) {
            ...
            try {
                ...
                const result = await whatsappClient.sendImage(recipientId, imageUrl || imageBase64, caption, !!imageUrl);
                return JSON.stringify(result);
            }
            catch (err) {
        ...
        case 'whatsapp_send_document': {
            const { recipient, documentUrl, documentBase64, filename } = args;
            if (!recipient) {
                return JSON.stringify({ error: 'recipient is required' });
            }
            ...
            try {
                ...
                const result = await whatsappClient.sendDocument(recipientId, documentUrl || documentBase64, filename, !!documentUrl);
                return JSON.stringify(result);
            }
            catch (err) {
        ...
        case 'whatsapp_transcribe_audio': {
            const { messageId, chatId, contactName } = args;
            if (!whatsappClient.isClientReady()) {
                return JSON.stringify({ error: 'WhatsApp is not connected. Check status first.' });
            }
